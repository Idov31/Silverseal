...
pub fn grub_arch_efi_linux_boot_image_hook(
    ...
    let target = unsafe { GRUB_ARCH_EFI_LINUX_BOOT_IMAGE_HOOK_INLINE.target };
    let original_bytes = unsafe { GRUB_ARCH_EFI_LINUX_BOOT_IMAGE_HOOK_INLINE.original_bytes };
    ...
    // TODO: Deploy another hook here to overrwrite specific kernel handler.

    // Jump to the original function.
    let original_fn: extern "C" fn(usize, usize, *const u8) -> i32 =
        unsafe { core::mem::transmute(GRUB_ARCH_EFI_LINUX_BOOT_IMAGE_HOOK_INLINE.target) };
